...
class AccountPage(QWidget):
    ...
    def _make_group(title: str) -> QGroupBox:
        ...
        group.setStyleSheet(
            ...
            f"padding: 0 6px; background: {_SURFACE}; }}"
            f"QGroupBox QCheckBox {{ color: {_TEXT}; background: transparent; }}"
            f"QGroupBox QCheckBox::indicator {{ width: 16px; height: 16px; "
            f"border: 2px solid {_SUBTEXT}; border-radius: 3px; background: {_BG}; }}"
            f"QGroupBox QCheckBox::indicator:checked {{ background: {_ACCENT}; "
            f"border-color: {_ACCENT}; }}"
            f"QGroupBox QLabel {{ background: transparent; }}"
            f"QGroupBox QSpinBox, QGroupBox QDoubleSpinBox {{ "
            f"background: {_BG}; color: {_TEXT}; border: 1px solid {_SUBTEXT}; "
            f"border-radius: 4px; padding: 4px 6px; }}"
            f"QGroupBox QSpinBox::up-button, QGroupBox QDoubleSpinBox::up-button, "
            f"QGroupBox QSpinBox::down-button, QGroupBox QDoubleSpinBox::down-button {{ "
            f"background: {_BG}; border: none; }}"
            f"QGroupBox QSpinBox::up-arrow, QGroupBox QDoubleSpinBox::up-arrow {{ "
            f"image: none; border-left: 4px solid transparent; border-right: 4px solid transparent; "
            f"border-bottom: 5px solid {_TEXT}; width: 0; height: 0; }}"
            f"QGroupBox QSpinBox::down-arrow, QGroupBox QDoubleSpinBox::down-arrow {{ "
            f"image: none; border-left: 4px solid transparent; border-right: 4px solid transparent; "
            f"border-top: 5px solid {_TEXT}; width: 0; height: 0; }}"
        )
        return group
